...
async def show_handle(bot: Bot, event: Event):
    ...
    if resp.empty:
        ...
        if '图' in text or '妹' in text or '女' in text or 'mm' in text or 'setu' in text:
            await show.finish(message=Message("阿y未找到该股票"))
        await show.finish(message=Message("阿y未查到该股票相关信息!"))
    name = resp['股票名称'][0]
    name = name.replace(' ', '')
...
async def gpjk_handle(bot: Bot, event: Event):
    ...
    if resp.empty:
        await gpjk.finish(message=Message("阿y未查到该股票相关信息!"))
    name = resp['股票名称'][0]
    ...
    await gpjk.send(
        message=Message(
            f"开始股票监控，{name} 当前净值为 {pre:.2f} ，涨跌幅为 {dis:+.2f}%，若变化超过 {bound:.2f} 个点，阿y我会告诉你的~[CQ:face,id=317]"))
    pre = dis  # 代表上次更新的涨跌幅
    prel = -1  # 上次更新时的长度
    change_time = 5  # 睡眠的时间
    cnt = 0
